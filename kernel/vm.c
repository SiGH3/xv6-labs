...
//   memset(kernel_pagetable, 0, PGSIZE);
 ...
 void ly_kvm_map_pagetable(pagetable_t pgtbl){
  ...
  // CLINT仅在内核启动时需要使用，而用户进程在内核态中的操作并不需要用到该映射
  // 该映射会与要map的程序内存冲突
...
int
ly_kvmcopymappings(pagetable_t src, pagetable_t dst, uint64 start, uint64 sz){
  pte_t* pte;
  uint64 pa, i;
  uint flags;
  ...
  for(i = PGROUNDUP(start);i<start+sz;i+=PGSIZE){
    // PGROUNDUP(start)：向上对齐到页边界，确保 start 不是中间地址，防止重复映射已存在的页
    // i 以页大小 (PGSIZE) 递增，逐页处理
    if((pte = walk(src,i,0))==0){   //在 src 页表中查找不到虚拟地址 i 对应的页表项
      panic("kvmcopymappings: pte should exist");
    }
    if((*pte &PTE_V)==0){
      panic("kvmcopymappings: page not present");
    }          //若 pte 不存在或无效，则 panic，说明 src 页表的该地址应该已经被映射
               //（否则不应该调用 ly_kvmcopymappings）
    
    pa =PTE2PA(*pte);   //提取页表项中的物理地址

    //& ~PTE_U 表示将该页的权限设置为非用户页
    //必须设置权限，因为RISC-V中内核是无法直接访问用户页的
    flags = PTE_FLAGS(*pte) & ~PTE_U;
    if(mappages(dst,i,PGSIZE,pa,flags)!=0){  //将 pa 物理地址映射到 dst 页表中的 i 位置，并赋予 flags 权限
      goto err;   //若映射失败，跳转到err
    }
  }
  return 0;
...
err:
  ...
  return -1;
  //解除 dst 中已经映射的页，do_free=0，仅删除映射，不释放物理内存
}


//与uvmdealloc功能类似，将程序内存从oldsz缩减到newsz，但不释放实际内存
uint64
ly_kvmdealloc(pagetable_t pagetable, uint64 oldsz, uint64 newsz)
{
  if(newsz >= oldsz)
    return oldsz;
...
void
ly_kvm_free_kernelpgtbl(pagetable_t pagetable){
  for(int i = 0; i<512; i++){
    pte_t pte = pagetable[i];
    uint64 child = PTE2PA(pte);
    if((pte&PTE_V)&&(pte&(PTE_R | PTE_W | PTE_X)) == 0){  // 如果该页表项指向更低一级的页表
      ly_kvm_free_kernelpgtbl((pagetable_t)child);        // 递归释放更低一级的页表及其页表项
      pagetable[i]=0;
    }
  }
